...
function showPosition(position) {
  var locations = [
   @foreach($rto as $row)
 ['{{$row->name}}','{{ $row->lattitude}}','{{ $row->longtitude}}','<img src="http://127.0.0.1:8000/agent_img/{{ $row->photo }}" height="75" width="75" />','{{ $row->contact }}','{{ $row->email }}'],
  @endforeach
     ];
    ...
    for (i = 0; i < locations.length; i++) {  
      marker = new google.maps.Marker({
        ...
        map: map,
        icon : "{{URL::to('/')}}/Userassets/rto.png"
      });

      google.maps.event.addListener(marker, 'click', (function(marker, i) {
        return function() {
          infowindow.setContent("<h4 align='center'>"+locations[i][0]+"</h4>"+"<br/>"+"<center>"+locations[i][3]+"</center>"+"<br/>"+"<center>"+locations[i][4]+"<center>"+"<br/>"+"<center>"+locations[i][5]+"<center>"+"<br/>");
...
<!-- Main Container  -->
 ...
 <div class="container">
            ...
            preset03-2 preset04-2 preset05-1">
               ...
               <h1 class="title-category"><span>RTO Agents</span></h1>
