                        ...
                        <div class="row">
                            <div class="content-product-left col-md-6 col-sm-12 col-xs-12">
                               <div class="large-image  vertical">
                                    <img itemprop="image" class="product-image-zoom" src="{{URL::to('/')}}/inscomp_img/{{ $d->logo }}"  title="{{ $d->com_name }}" alt="{{ $d->com_name }}">
                                </div>
                            </div>

                            <div class="content-product-right col-md-6 col-sm-12 col-xs-12">
                                <div class="title-product">
                                   <a href="/Users/InsuranceCompany/insucompanydetail/{{$d->ins_id}}"> <h1>{{ $d->com_name }}</h1></a>
                                </div>
                                ...
                                <div class="product-box-desc">
                                    <div class="inner-box-desc">
                                        <div class="price-tax"><span>Policy name:</span>{{ $d->p_name }}</div>
                                        <div class="reward"><span>Contact :</span> {{ $d->contact }}</div>
                                        <div class="brand"><span>Email:</span>{{ $d->email }}</div>
                                        <div class="model"><span>Website:</span> {{ $d->website }} </div>
                                        <div class="reward"><span>Addressline1:</span> {{ $d->addressline1 }} </div>
                                        <div class="reward"><span>Addressline2:</span> {{ $d->addressline2 }} </div>
                                        <div class="reward"><span>Landmark:</span> {{ $d->landmark }} </div>
                                        <div class="reward"><span>Pincode:</span> {{ $d->pincode }} </div>
                                        <div class="reward"><span>City Name:</span> {{ $d->city_name }} </div>
                                        <div class="reward"><span>About:</span> {{ $d->about }} </div>
